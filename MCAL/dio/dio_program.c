...
EN_dioError_t MDIO_SetPinDirection (u8 u8_arg_portNum,u8 u8_arg_pinNum,u8 u8_arg_pinDir)
{
	...
	EN_dioError_t enum_Local_errorStatus = DIO_OK;
	
	if (u8_arg_pinNum >= NUM_OF_PINS_PER_PORT || u8_arg_portNum >= NUM_OF_PORTS)
    {
				enum_Local_errorStatus = DIO_NOT_OK;
    }
	else
	{
...
u8   MDIO_u8GetPinValue       (u8 u8_arg_portNum,u8 u8_arg_pinNum)
{
	...
	else
	{
		...
		{
		case PORTA:
			u8_Local_Data=GET_BIT(MDIO_PINA,u8_arg_pinNum);
			break; 
    
		case PORTB:
			u8_Local_Data=GET_BIT(MDIO_PINB,u8_arg_pinNum);
			break; 

		case PORTC:
			u8_Local_Data=GET_BIT(MDIO_PINC,u8_arg_pinNum);
			break;

		case PORTD:
			u8_Local_Data=GET_BIT(MDIO_PIND,u8_arg_pinNum);
			break;
...
EN_dioError_t MDIO_SetPortDirection (u8 u8_arg_portNum,u8 u8_arg_pinDir)
{
	...
	else
	{
		...
		{
			case PORTA:  
				MDIO_DDRA = u8_arg_pinDir;
				break;
			
			case PORTB:  
				MDIO_DDRB = u8_arg_pinDir; 
				break;

			case PORTC:  
				MDIO_DDRC = u8_arg_pinDir; 
				break;

			case PORTD:  
				MDIO_DDRD = u8_arg_pinDir; 
				break;
...
u8 MDIO_GetPortValue (u8 u8_arg_portNum)
{
	...
	else
	{
		...
		{
		case PORTA: 
			u8_Local_Data = MDIO_PINA; 
			break; 
    
		case PORTB: 
			u8_Local_Data = MDIO_PINB; 
			break; 

		case PORTC: 
			u8_Local_Data = MDIO_PINC; 
			break; 

		case PORTD: 
			u8_Local_Data = MDIO_PIND; 
			break; 
		default:
			break;
		}
